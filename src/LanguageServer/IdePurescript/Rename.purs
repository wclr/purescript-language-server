...
-- import LanguageServer.IdePurescript.Symbols (getLocalRanges)
...
renameIdentifier docs settings state ({ textDocument, position, newName }) = do
  ...
  case identInfo of
    Just ({ word, found: Right { typeInfo, usages } }) -> do
      ...
      liftEffect $ do
        usgEdits <-
          getTextEdits typeInfo usages docsToEdit newName word
        pure
          $ makeMultiWorkspaceEdit clientCapabilities (toMultiEdits usgEdits)
    Just ({ word, found: Left { ranges } }) -> do
      pure wsEditEmpty
  ...
  goFile dcs path = do
    ...
    pure $ Map.insert path
      { uri
      , docTextAtRange:
          pure <<< getTextAtRangeInLines (splitLines docText)
      , version
...
type DocToEdit =
  { docTextAtRange :: Range -> Effect String
  , uri :: DocumentUri
...
getTextEdits typeInfo usages docsToEdit newText oldName =
      ...
      ( \editsMap tp@(TypePosition { name }) ->
          ...
          in
            case Map.lookup name docsToEdit of
              Nothing ->
                pure editsMap
              Just { docTextAtRange, uri, version } -> do
                rangeText <- docTextAtRange useRange
                let
                  -- Get qualifier index.
                  qualIdx =
                    maybe 0 ((+) 1) $ String.indexOf (Pattern ".") rangeText
                ...
                pure $ maybe cancelEdit (addEdit') mbRange $ editsMap
      )
      edits
      usages
  ...
  addTypeDefEdit edits =
    case getDefined typeInfo of
      Nothing ->
        pure edits
      Just (tp /\ { docTextAtRange, version, uri, parsed }) -> do
        let
          defRange = rangeFromTypePos tp
          addEdit' range = addEdit (uri /\ version) { range, newText }
          fixRange range = do
            rangeText <- docTextAtRange range
            let mbRange = findWordInRange (Pattern oldName) rangeText range
            pure $ maybe cancelEdit addEdit' mbRange
        (pure edits)
          # ap (fixRange defRange)
          ...
          # ap
              ( maybeParseResult (pure identity)
                  ( \m ->
    ...
    Array.foldM
      ( \editsMap fileName ->
          case Map.lookup fileName docsToEdit of
            Just { uri, version, parsed, docTextAtRange } -> do
              let
                addEdit' range = addEdit (uri /\ version) { range, newText }
                fixRange range = do
                  rangeText <- docTextAtRange range
...
getTypeInfoWithUsages port word wordPos qualifier moduleState = do
  ...
  -- case findInDefined (spy "typeInfos" typeInfos) of
  --   Just info ->
  --     getUsages info <#> Just <<< (/\) info
  --   Nothing ->
  --     -- Find usage corresponding to target.
  --     Array.foldM
  --       ( \res info ->
  --           maybe (getCheckedUsages info) (pure <<< Just) res
  --       )
  --       Nothing
  --       typeInfos
  where
